...
> {
  ...
  public constructor(
    ...
    if (tradeType === TradeType.EXACT_INPUT) {
      ...
      tokenAmounts[tokenAmounts.length - 1] = wrappedCurrencyAmount(
        amount,
        route.chainId,
      );
  ...
  >(
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      // pair irrelevant
      if (
        !currencyEquals(pair.token0, amountIn.currency) &&
        !currencyEquals(pair.token1, amountIn.currency)
      )
        continue;
  ...
  >(
    ...
    for (let i = 0; i < pairs.length; i++) {
      ...
      // pair irrelevant
      if (
        !currencyEquals(pair.token0, amountOut.currency) &&
        !currencyEquals(pair.token1, amountOut.currency)
      )
        continue;
      if (pair.reserve0.equalTo(ZERO) || pair.reserve1.equalTo(ZERO)) continue;
